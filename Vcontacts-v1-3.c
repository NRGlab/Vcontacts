 ...
 **********************************************************************/
 ...
 /**********************************************************************
 * Modified by LP More on 10/12/2012 for added SYBYL atom types names and
 * numbers in the output in the case of RNA and nucleotides A,C,G,U. 
...
struct ptindex    ptorder[11000];  // for ordering vertices around each face // RJN 081008 changed 100 to 1100 and 200 2200 
struct vertex     centerpt[11000]; // center points for each contact face    // RJN 081008
struct vertex     poly[22000];     // polyhedron vertices                    // RJN 081008
struct plane      cont[11000];     // atom and contact plane information     // RJN 081008
struct edgevector vedge[22000];
...
char      include_water;         // do not exclude HOH molecules from contacts
long int *seed;                  // seed vertices for new polyhedra
char     radfilename[250];       // custom name for radii.dat file including path ADDED RJN 28.04.2008
// --------------------- function prototypes ----------------------
...
void   atom_type_definition(struct atom *PDB, int atomi);
...
int main(int argc, char *argv[])
{
  ...
  char     FILENAME[250];    // input file name
...
int calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms)
{
  ...
  for(atomi=0; atomi<PDBtot; ++atomi) {
    ...
    printf(" %5d%5s %4d%5s %c     SAS   %7.2f    %5s %2d\n", PDB[atomi].atomnum,       // ADDED RJN 29.04.2008
	   PDB[atomi].atomname, PDB[atomi].resnum, PDB[atomi].res,
	   PDB[atomi].chn,                                                        // ADDED RJN 29.04.2008
	   (normalize=='Y')?(PDB[atomi].SAS*100.0/(rado*rado*4.0*PI)):PDB[atomi].SAS, PDB[atomi].atomtypename, PDB[atomi].atomtypenum);  
	...
	printf("                      %5d%5s %4d%5s %c %5s %2d %7.2f  %4.2f\n",                   // ADDED RJN 29.04.2008
	       ...
	       PDB[ca_rec[currindex].atom].chn, PDB[atomi].atomtypename, PDB[atomi].atomtypenum,                                          // ADDED RJN 29.04.2008
	...
	if(PDB[atomi].resnum == PDB[ca_rec[currindex].atom].resnum &&                       // ADDED FG 19.11.2010
	   PDB[atomi].chn == PDB[ca_rec[currindex].atom].chn)
  {  
	  ...
	  //PDB[atomi].resnum == PDB[ca_rec[currindex].atom].resnum &&                       // ADDED FG 19.11.2010
	  //PDB[atomi].chn == PDB[ca_rec[currindex].atom].chn)                               // ADDED FG 19.11.2010
	...
	printf("                      %5d%5s %4d%5s %c %5s %2d %7.2f  %4.2f\n",                    // ADDED RJN 29.04.2008
	       PDB[ca_rec[currindex].atom].atomnum, PDB[ca_rec[currindex].atom].atomname,
	       PDB[ca_rec[currindex].atom].resnum, PDB[ca_rec[currindex].atom].res,      
	       PDB[ca_rec[currindex].atom].chn, PDB[ca_rec[currindex].atom].atomtypename, PDB[ca_rec[currindex].atom].atomtypenum,                                          // ADDED RJN 29.04.2008
	       ca_rec[currindex].area, ca_rec[currindex].dist);                          
	  ++contnum;
      }
      currindex = ca_rec[currindex].prev;
...
struct atom *read_PDB(char *FILE_ptr, struct atom *PDB, long int *numPDBatoms)
{
  ...
  while (1) {
    ...
    if ((strncmp(oneline, "END",3)==0) || (oneline_ptr == NULL)) {
      fclose(infile);
      *numPDBatoms = PDBtot;
      // atom_type_definition(PDB, PDBtot);   # Placed the function's call in assign_radii() for now, let's check performance issues related to that. LP 10/12/12
    ...
    //if (!strncmp(&oneline[0], "ATOM  ", 6)) {
      ...
      // ------- skip HOH records ----------
      if (include_water == 'N' && !strncmp(&oneline[17], "HOH", 3)) continue;
...
void assign_radii(struct atom *PDB, long int PDBtot)
{
	...
	for(atomi=0; atomi<PDBtot; ++atomi) {
		...
		if  (!strncmp(PDB[atomi].res, "  C", 3) || !strncmp(PDB[atomi].res, "  G", 3) || !strncmp(PDB[atomi].res, "  A", 3) || !strncmp(PDB[atomi].res, "  T", 3) || !strncmp(PDB[atomi].res, "  U", 3) 
      || !strncmp(PDB[atomi].res, " C ", 3) || !strncmp(PDB[atomi].res, " G ", 3) || !strncmp(PDB[atomi].res, " A ", 3) || !strncmp(PDB[atomi].res, " T ", 3) || !strncmp(PDB[atomi].res, " U ", 3) 
      || !strncmp(PDB[atomi].res, "C  ", 3) || !strncmp(PDB[atomi].res, "G  ", 3) || !strncmp(PDB[atomi].res, "A  ", 3) || !strncmp(PDB[atomi].res, "T  ", 3) || !strncmp(PDB[atomi].res, "U  ", 3)
      || !strncmp(PDB[atomi].res, "ALA", 3) || !strncmp(PDB[atomi].res, "ARG", 3) || !strncmp(PDB[atomi].res, "ASN", 3) || !strncmp(PDB[atomi].res, "ASP", 3) || !strncmp(PDB[atomi].res, "CYS", 3)
      || !strncmp(PDB[atomi].res, "GLN", 3) || !strncmp(PDB[atomi].res, "GLU", 3) || !strncmp(PDB[atomi].res, "GLY", 3) || !strncmp(PDB[atomi].res, "HIS", 3) || !strncmp(PDB[atomi].res, "ILE", 3)
      || !strncmp(PDB[atomi].res, "LEU", 3) || !strncmp(PDB[atomi].res, "LYS", 3) || !strncmp(PDB[atomi].res, "MET", 3) || !strncmp(PDB[atomi].res, "PHE", 3) || !strncmp(PDB[atomi].res, "PRO", 3)
      || !strncmp(PDB[atomi].res, "SER", 3) || !strncmp(PDB[atomi].res, "THR", 3) || !strncmp(PDB[atomi].res, "TRP", 3) || !strncmp(PDB[atomi].res, "TYR", 3) || !strncmp(PDB[atomi].res, "VAL", 3)) 
    {
			atom_type_definition(PDB, atomi);
...
void parse_commandline(int argc, char *argv[], char FILENAME[])
{
	...
	planedef = 'X';    // default
	normalize = 'N';   // default
	include_water = 'N'; // ADDED FG 102110
	...
	while(argi < argc) {
		if(!strncmp(argv[argi], "-all", 4)) {
		        ...
		        normalize = 'Y';
		} else if(!strncmp(argv[argi], "-hoh", 4)) {
		        include_water = 'Y';
...
void atom_type_definition(struct atom *PDB,int atomi)
{
  if (!strncmp(PDB[atomi].res," G ",3) || !strncmp(PDB[atomi].res,"G  ",3) || !strncmp(PDB[atomi].res,"  G",3))
  {
      if (strncmp(PDB[atomi].atomname, " P  ",4) == 0) {strncpy(PDB[atomi].atomtypename, "P.3",5); PDB[atomi].atomtypenum = 22;}
      if (strncmp(PDB[atomi].atomname, " O1P",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.2",5); PDB[atomi].atomtypenum = 13;}
      if (strncmp(PDB[atomi].atomname, " O2P",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.co2",5); PDB[atomi].atomtypenum = 15;}
      if (strncmp(PDB[atomi].atomname, " C5'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " O5'",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
      if (strncmp(PDB[atomi].atomname, " C4'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " O4'",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
      if (strncmp(PDB[atomi].atomname, " C3'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " O3'",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
      if (strncmp(PDB[atomi].atomname, " C2'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " O2'",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
      if (strncmp(PDB[atomi].atomname, " C1'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " N1 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
      if (strncmp(PDB[atomi].atomname, " C2 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
      if (strncmp(PDB[atomi].atomname, " N2 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.pl3",5); PDB[atomi].atomtypenum = 12;}
      if (strncmp(PDB[atomi].atomname, " N3 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
      if (strncmp(PDB[atomi].atomname, " C4 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
      if (strncmp(PDB[atomi].atomname, " C5 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
      if (strncmp(PDB[atomi].atomname, " C6 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
      if (strncmp(PDB[atomi].atomname, " O6 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.2",5); PDB[atomi].atomtypenum = 13;}
      if (strncmp(PDB[atomi].atomname, " N7 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.pl3",5); PDB[atomi].atomtypenum = 12;}
      if (strncmp(PDB[atomi].atomname, " C8 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
      if (strncmp(PDB[atomi].atomname, " N9 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.pl3",5); PDB[atomi].atomtypenum = 12;}
  }
  if (!strncmp(PDB[atomi].res," C ",3) || !strncmp(PDB[atomi].res,"C  ",3) || !strncmp(PDB[atomi].res,"  C",3))
  {
    if (strncmp(PDB[atomi].atomname, " P  ",4) == 0) {strncpy(PDB[atomi].atomtypename, "P.3",5); PDB[atomi].atomtypenum = 22;}
    if (strncmp(PDB[atomi].atomname, " O1P",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname, " O2P",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.co2",5); PDB[atomi].atomtypenum = 15;}
    if (strncmp(PDB[atomi].atomname, " C5'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname, " O5'",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname, " C4'",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.3",5); PDB[atomi].atomtypenum = 3;}
    ...
    if (strncmp(PDB[atomi].atomname, " N1 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
    if (strncmp(PDB[atomi].atomname, " C2 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname, " O2 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname, " N3 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
    if (strncmp(PDB[atomi].atomname, " C4 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname, " N4 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "N.pl3",5); PDB[atomi].atomtypenum = 12;}
    if (strncmp(PDB[atomi].atomname, " C5 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname, " C6 ",4) == 0) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
  }
  if (!strncmp(PDB[atomi].res," A ",3) || !strncmp(PDB[atomi].res,"A  ",3) || !strncmp(PDB[atomi].res,"  A",3))
  {
     if (strncmp(PDB[atomi].atomname, " P  ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "P.3",5); PDB[atomi].atomtypenum = 22;}
     if (strncmp(PDB[atomi].atomname, " O1P",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "O.3",5); PDB[atomi].atomtypenum = 14;}
     if (strncmp(PDB[atomi].atomname, " O2P",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "O.co2",5); PDB[atomi].atomtypenum = 15;}
     ...
     if (strncmp(PDB[atomi].atomname, " C5 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
     if (strncmp(PDB[atomi].atomname, " C6 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
     if (strncmp(PDB[atomi].atomname, " N6 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "N.pl3",5); PDB[atomi].atomtypenum = 12;}
     if (strncmp(PDB[atomi].atomname, " N7 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
     if (strncmp(PDB[atomi].atomname, " C8 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "C.ar",5); PDB[atomi].atomtypenum = 4;}
     if (strncmp(PDB[atomi].atomname, " N9 ",4) == 0 ) {strncpy(PDB[atomi].atomtypename, "N.ar",5); PDB[atomi].atomtypenum = 10;}
  }
  if (!strncmp(PDB[atomi].res," U ",3) || !strncmp(PDB[atomi].res,"U  ",3) || !strncmp(PDB[atomi].res,"  U",3))
  {
    if (strncmp(PDB[atomi].atomname," P  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"P.3",5); PDB[atomi].atomtypenum = 22;}
    if (strncmp(PDB[atomi].atomname," O1P",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.3",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname," O2P",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.co2",5); PDB[atomi].atomtypenum = 15;}
    ...
    if (strncmp(PDB[atomi].atomname," O2 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," N3 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.ar",5); PDB[atomi].atomtypenum = 10;}
    if (strncmp(PDB[atomi].atomname," C4 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname," O4 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," C5 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname," C6 ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 14;}
  }
  if(!strncmp(PDB[atomi].res,"ALA",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
  }
  if(!strncmp(PDB[atomi].res,"ARG",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," NE ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.pl3",5); PDB[atomi].atomtypenum = 12;}
    if (strncmp(PDB[atomi].atomname," CZ ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.cat",5); PDB[atomi].atomtypenum = 5;}
    if (strncmp(PDB[atomi].atomname," NH1",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.pl3",5); PDB[atomi].atomtypenum = 12;}
    if (strncmp(PDB[atomi].atomname," NH2",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.pl3",5); PDB[atomi].atomtypenum = 12;}
  }
  if(!strncmp(PDB[atomi].res,"ASN",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," OD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," ND2",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
  }
  if(!strncmp(PDB[atomi].res,"ASP",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," OD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.co2",5); PDB[atomi].atomtypenum = 15;}
    if (strncmp(PDB[atomi].atomname," OD2",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.co2",5); PDB[atomi].atomtypenum = 15;}
  }
  if(!strncmp(PDB[atomi].res,"CYS",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," SG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"S.3",5); PDB[atomi].atomtypenum = 18;}
  }
  if(!strncmp(PDB[atomi].res,"GLN",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," OE1",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," NE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
  }
  if(!strncmp(PDB[atomi].res,"GLU",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," OE1",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.co2",5); PDB[atomi].atomtypenum = 15;}
    if (strncmp(PDB[atomi].atomname," OE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.co2",5); PDB[atomi].atomtypenum = 15;}
  }
  if(!strncmp(PDB[atomi].res,"GLY",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
  ...
  if(!strncmp(PDB[atomi].res,"HIS",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," ND1",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.ar",5); PDB[atomi].atomtypenum = 10;}
    if (strncmp(PDB[atomi].atomname," CD2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," NE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.ar",5); PDB[atomi].atomtypenum = 10;}
  }
  if(!strncmp(PDB[atomi].res,"ILE",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
  }
  if(!strncmp(PDB[atomi].res,"LEU",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
  }
  if(!strncmp(PDB[atomi].res,"LYS",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CD ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CE ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," NZ ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.4",5); PDB[atomi].atomtypenum = 9;}
  }
  if(!strncmp(PDB[atomi].res,"MET",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," SD ",4) == 0) {strncpy(PDB[atomi].atomtypename,"S.3",5); PDB[atomi].atomtypenum = 18;}
    if (strncmp(PDB[atomi].atomname," CE ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
  }
  if(!strncmp(PDB[atomi].res,"PHE",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," CG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CD2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CE1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CZ ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
  }
  if(!strncmp(PDB[atomi].res,"PRO",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
  ...
  if(!strncmp(PDB[atomi].res,"SER",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," OG ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.3",5); PDB[atomi].atomtypenum = 14;}
  }
  if(!strncmp(PDB[atomi].res,"THR",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    if (strncmp(PDB[atomi].atomname," O  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.2",5); PDB[atomi].atomtypenum = 13;}
    if (strncmp(PDB[atomi].atomname," CB ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," OG1",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.3",5); PDB[atomi].atomtypenum = 14;}
    if (strncmp(PDB[atomi].atomname," CG2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
  }
  if(!strncmp(PDB[atomi].res,"TRP",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CD1",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CD2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," NE1",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.ar",5); PDB[atomi].atomtypenum = 10;}
    if (strncmp(PDB[atomi].atomname," CE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CE3",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CZ2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CZ3",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CH2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
  }
  if(!strncmp(PDB[atomi].res,"TYR",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
    ...
    if (strncmp(PDB[atomi].atomname," CE2",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," CZ ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.ar",5); PDB[atomi].atomtypenum = 4;}
    if (strncmp(PDB[atomi].atomname," OH ",4) == 0) {strncpy(PDB[atomi].atomtypename,"O.3",5); PDB[atomi].atomtypenum = 14;}
  }
  if(!strncmp(PDB[atomi].res,"VAL",3))
  {
    if (strncmp(PDB[atomi].atomname," N  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"N.am",5); PDB[atomi].atomtypenum = 11;}
    if (strncmp(PDB[atomi].atomname," CA ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.3",5); PDB[atomi].atomtypenum = 3;}
    if (strncmp(PDB[atomi].atomname," C  ",4) == 0) {strncpy(PDB[atomi].atomtypename,"C.2",5); PDB[atomi].atomtypenum = 2;}
